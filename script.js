...
function renderSnacks() {
    ...
    filtered.forEach(item => {
        ...
        li.innerHTML = `
            <span onclick="openSnackModal('${item.name}')" style="font-weight:700; cursor:pointer;">${item.name}</span>
...
async function handleLogin() {
    ...
    if (!userData) return alert("정보가 없습니다.");
    if (userData.pw !== await hashPassword(pw)) return alert("비밀번호 오류");
...
async function handleSignup() {
    ...
    currentUser = { name, pw: hashed, loginCount: 1, favorites: [], allergies: [], ratings: {} };
    await saveUserData();
    closeModal();
    updateUI();
}
...
window.onclick = (e) => {
    if (e.target.classList.contains('gh-modal')) { closeModal(); closeSnackModal(); }
};

// 기타 함수들 (pickRandom, logout, addFavorite 등 기존 코드 유지)
function pickRandom() {
    const items = document.querySelectorAll(".gh-snack-item span");
    if (!items.length) return alert("없음");
    const picked = items[Math.floor(Math.random() * items.length)].innerText;
    document.getElementById("result").innerHTML = `✨ 오늘의 추천: <br><b>[ ${picked} ]</b>`;
}
...
function renderAllergyList() {
    ...
    container.innerHTML = allergyTypes.map(type => {
        const checked = currentUser.allergies.includes(type);
        return `<label class="gh-chip ${checked ? 'active' : ''}"><input type="checkbox" hidden value="${type}" ${checked ? 'checked' : ''} onchange="updateAllergy(this)"> ${type}</label>`;
    }).join('');
}
function updateAllergy(el) {
    ...
    saveUserData(); renderSnacks(); renderAllergyList();
}
